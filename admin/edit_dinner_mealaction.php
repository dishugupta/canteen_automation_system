...
if(isset($_POST['submit']))
{
	...
	$data			=	mysql_query("SELECT `name` FROM `dinnermenu` WHERE `name`='$name'");
	...
	{
		$_SESSION['id']	=	$id;
		@header("location:edit_dinner_meal.php");
		exit;
	}
	else
	{
		mysql_query("UPDATE `dinnermenu` SET `name`='".$name."',`menu`='".$menu."',`dayrate`='".$dayrate."',`monthrate`='".$monthrate."',`description`='".$description."' WHERE `id`='$id'")or die(mysql_error());
		
		$_SESSION['success']	=	"dinner meal pack updated.";
		@header("location:show_dinner_meal.php");
		exit;
	}
}
else
